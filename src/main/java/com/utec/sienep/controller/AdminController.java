...
@Tag(name = "Administración", description = "Gestión de roles, permisos y usuarios")
...
public class AdminController {
    ...
    public ResponseEntity<ApiResponseDTO<Rol>> crearRol(
        ...
        return ResponseEntity.status(HttpStatus.CREATED).body(ApiResponseDTO.ok("Rol creado.", adminService.crearRol(nombre, descripcion)));
    }

    // Listar usuarios
    @GetMapping("/usuarios")
    @Operation(summary = "Listar usuarios del sistema", description = "La respuesta nunca incluye contraseñas ni hashes.")
    public ResponseEntity<ApiResponseDTO<List<UsuarioResponseDTO>>> listarUsuarios() {
    ...
    @Operation(summary = "Desbloquear usuario", description = "Desbloquea una cuenta que fue bloqueada por exceder el máximo de intentos fallidos.")
    public ResponseEntity<ApiResponseDTO<Void>> desbloquear(
            @PathVariable Long usuarioId, @AuthenticationPrincipal UserDetails admin) {
